...
class Log
{
    ...
    public static function record(string $message, string $level = self::ERR)
    {
        ...
        try {
            ...
            // 生成日志文件
            self::create($fileName, self::$basePath, $day, $level);
     ...
     * @param $day
     */
    private static function create($fileName, $path, $day, $level)
    {
        ...
        $size = 20 * 1024 * 1024;
        if ($fileSize > $size) {
            $renameResult = self::rename($fileName, $path, $day, $level);
            if ($renameResult) {
                self::createAndAuth($fileName);
            }
        }
     ...
     * @return bool
     */
    private static function rename($fileName, $path, $day, $level)
    {
        $result = false;
        try {
            $allFile = scandir($path . '/log/' . $day);
            if (is_array($allFile) && !empty($allFile)) {
                $fileString = implode('', $allFile);
                preg_match_all("/{$level}-[\d]/", $fileString, $matches);
                $fileCount = count($matches[0]);
                $fileNumber = $fileCount + 1;
            } else {
                $fileNumber = 1;
            }
            $time = date('His', time());
            $newFileName = $path . '/log/' . $day . '/' . $level . '-' . $fileNumber . '-' . $time . '.log';
